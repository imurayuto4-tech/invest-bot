...
# --- 短期モメンタム枠 ---
SLEEVE_PCT = 0.97         # ロングの投下上限(資産の約97%。ショート撤廃後はリターン重視でフルに近く)
TOP_N = 10                # 新規建ての目標銘柄数(条件を満たした分だけ最大10。少なければ自動で現金多め)
KEEP_N = 10               # 入替ヒステリシス幅(=TOP_Nでヒステリシス無効。守りはEXIT_SMAのトレンド割れ決済に任せる)
MOMENTUM_DAYS = 20        # 短期化(60→20日モメンタム)。数日〜2週間のスイング
EXIT_SMA = 20             # 個別トレンド割れ判定: 保有が自分の20日線を割ったら市場OKでも降りる(唯一の実質的な守り)
STOP_LOSS_PCT = 8.0       # STOP_MODE="fixed" のときの一律損切り幅 / atrの保険値

# --- ショート(空売り) ---
# ★2026-07-15 撤廃★ バックテスト(2018-2026)でショートが致命的と判明:
#   ショート有り Sharpe0.3〜0.4/最大DD-53〜57% vs ロングのみ Sharpe1.0〜1.15/DD-20〜24%。
#   上昇相場で弱い銘柄を空売り→踏み上げで刈られる典型。よって下落局面は空売りせず現金退避。
ALLOW_SHORT = False

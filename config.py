...
# --- 短期モメンタム枠 ---
SLEEVE_PCT = 0.80         # ロング/ショート合計で資産の約80%まで(残り20%は現金クッション=往復ダメージ軽減)
TOP_N = 10                # 新規建ての目標銘柄数(条件を満たした分だけ最大10。少なければ自動で現金多め)
KEEP_N = 15               # 入替ヒステリシス: 保有はtop15から外れるまで持ち続ける(回転抑制→ストップに仕事をさせる)
MOMENTUM_DAYS = 20        # 短期化(60→20日モメンタム)。数日〜2週間のスイング
EXIT_SMA = 20             # 個別トレンド割れ判定: 保有が自分の20日線を割ったら市場OKでも降りる
